...
bool verify_sig (string pubKey, std::string signature) {
    ...
    if (verified == 1) {
        ...
        cout << bstr2hexstr(serialized_txn, serialized_txn.length()) << endl;
        cout << pubKey.length() << ": " << pubKey << endl;
        cout << signature.length() << ": " << signature << "\n\n" << endl;
    }
    EC_KEY_free(publicKey); // Free allocated memory
...
bool exec_op (string op, stack<string> &stk, Json::Value &txn, int &bytes_to_push) {
    ...
    if (op == "OP_0") {
        ...
        // cout << "Pushing " << stoi(op.substr(11)) << " to stack" << endl;
        ...
        // cout << "Pushing " << bytes_to_push << " bytes to stack" << endl;
    }
    ...
    // -ve value of bytes_to_push means that we have to read the first "bytes_to_push" bytes to get the number of bytes to push
        ...
        // cout << "Duplicating top stack item" << endl;
        stk.push(stk.top());
    }
    else if (op == "OP_HASH160") {
        // cout << "Hashing the input twice: first with SHA-256 and then with RIPEMD-160" << endl;
        ...
        // cout << "Comparing the top two stack items. If they are not equal, the script fails" << endl;
        string top = stk.top();
        stk.pop();
        if (top == stk.top())
            stk.pop();
        ...
        // cout << "Verifying txn against signature and public key" << endl;
        ...
        // cout << "Verifying txn against signature and public key. If not valid, the script fails" << endl;
        string pubKey = stk.top();
        stk.pop();
        string sig = stk.top();
        stk.pop();
        ...
        // cout << "Marks transaction as invalid if top stack value is not true" << endl;
        if (stk.top() == "TRUE")
            stk.pop();
        else
            return false;
        ...
        // cout << "For each signature and public key pair, OP_CHECKSIG is executed. If all signatures are valid, the script returns true" << endl;
        ...
        // cout << "Dropping the top stack item" << endl;
        stk.pop();
    }
    else if (op == "OP_DEPTH") {
        // cout << "Pushing the number of stack items to stack" << endl;
